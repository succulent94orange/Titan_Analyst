...
def get_dynamic_prompts(ticker, current_price=None, current_yield=None):
    ...
    **SEC FORENSICS:** Simulate accessing SEC.gov (10-K/10-Q).
    ...
    4. **Historical Audit:** Would this model have failed in the last crisis (2020/2022) based on current logic.
...
# UPDATED CSS: Forces text wrapping for pre/code/p tags
STYLE_CSS = f"""
    @page {{
        size: letter;
        margin: 2cm;
    ...
    pre, code, p, div, span, li {{
        ...
        max-width: 100%;
        font-family: Helvetica, Arial, sans-serif; /* Override monospace defaults */
        font-size: 11pt;
    }}
    ...
    /* FINANCIAL TABLE STYLING - FIXED ALIGNMENT */
    table {{
        width: 100%;
    ...
    th {{
        ...
        color: {COLOR_GOLD}; /* Gold Text on Navy */
        font-weight: bold;
    ...
    td {{
        padding: 8px;
        border: 1px solid #ddd;
        text-align: right; /* Default to Right Align (Numbers) */
    }}
    td:first-child {{
        text-align: left; /* Force Left Align for the Label Column */
        font-weight: bold;
        color: {COLOR_NAVY};
    }}
    tr:nth-child(even) {{
        background-color: {COLOR_LIGHT_BLUE}; /* Alternating Rows */
    }}
...
def generate_pdf_report(ticker, report_data, chart_path=None, return_path=None, return_df=None):
    ...
    <body>
        <div id="header_content" style="text-align: center; color: {COLOR_NAVY}; font-weight: bold; border-bottom: 2px solid {COLOR_GOLD}; padding-bottom: 5px;">
            TITAN FINANCIAL ANALYST // {ticker} // EQUITY RESEARCH
        </div>
...
# --- 6. AGENT ENGINE (NEW SDK + RAG INJECTION) ---
async def run_agent(name, prompt, content):
    await asyncio.sleep(random.uniform(0.5, 2.0))
    for model_name in MODELS:
        try:
            # Correct google-genai SDK syntax for safety settings
            # We use a simplified config dictionary which the SDK parses
            ...
            if "429" in error_str:
                ...
                # If model not found, try next in list
                continue
            # If it's a safety/other error, allow retry with next model
            continue

    return name, f"Analysis Failed for {name}. Last Error: {error_str if 'error_str' in locals() else 'Unknown'}"

async def run_analysis(ticker):
    # Normalize ticker (handle user typing sentences)
    clean_ticker = normalize_ticker(ticker)
    
    # 1. LIVE DATA FETCH (RAG)
